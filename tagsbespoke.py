...
cur.execute("select department from public.holdertags where department = %s", ('tag', ))
...
pandas_df= pandas_df.withColumn('lastpaymentdate', date_format(pandas_df['lastpaymentdate'], dat_format))
pandas_df= pandas_df.withColumn('created', date_format(pandas_df['created'], dat_format))
pandas_df= pandas_df.withColumn("nextpaymentdate", date_format(pandas_df['nextpaymentdate'], dat_format))
pandas_df= pandas_df.withColumn("activationdate", date_format(pandas_df['activationdate'], dat_format))
pandas_df= pandas_df.withColumn("verificationaccesstime", date_format(pandas_df['verificationaccesstime'], dat_format))
...
cur.execute("insert into process_record(script_name, record) values(%s, %s)", ('tag', len(df1)))
conn.commit()
